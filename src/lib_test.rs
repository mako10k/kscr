...
#[test]
fn ir_run_main_user_defined_typeclass_imports_instance_transitive_nested_qualified() {
    use std::time::{SystemTime, UNIX_EPOCH};
    ...
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    let dir = std::env::temp_dir().join(format!(
        "kscr_typeclass_import_transitive_nested_qualified_{nanos}"
    ));
    std::fs::create_dir_all(&dir).unwrap();
...
#[test]
fn ir_run_main_user_defined_typeclass_imports_instance_transitive_deep_nested_qualified() {
    use std::time::{SystemTime, UNIX_EPOCH};
    ...
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    let dir = std::env::temp_dir().join(format!(
        "kscr_typeclass_import_transitive_deep_nested_qualified_{nanos}"
    ));
    std::fs::create_dir_all(&dir).unwrap();
...
#[test]
fn ir_run_main_user_defined_typeclass_imports_instance_transitive_deep_nested_reexported() {
    use std::time::{SystemTime, UNIX_EPOCH};
    ...
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    let dir = std::env::temp_dir().join(format!(
        "kscr_typeclass_import_transitive_deep_nested_reexported_{nanos}"
    ));
    std::fs::create_dir_all(&dir).unwrap();
