...
mod inference_tests {
    ...
    #[test]
    fn typecheck_file_resolves_method_value_with_transitive_instance_nested_qualified() {
        let dir = std::env::temp_dir().join(format!(
            "kscr_typecheck_file_transitive_method_value_nested_qual_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    #[test]
    fn typecheck_file_resolves_method_value_with_transitive_instance_deep_nested_reexported() {
        let dir = std::env::temp_dir().join(format!(
            "kscr_typecheck_file_transitive_method_value_deep_nested_reexport_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
