...
Initial scope is intentionally small: **only `Show`**, only for **primitive types**, and dictionary-passing in IR.
...
- Introduce `Constraint` type:
  - `enum Constraint { Show(Ty) }` (MVP: only Show)
...
- Solver (MVP): only handles `Show (Con "Integer")` etc.
...
- Add IR for dictionaries:
  - minimal: represent dictionaries as records: `{ show: a -> String }` encoded as `Value::Record`.
  - or add explicit `Value::DictShowInt`, etc. (simpler runtime, less general).
